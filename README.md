...
Direct URL access (no login). The admin portal has four tabs:
...
└── public/
    ...
    ├── customer.js           Customer-side frontend logic
...
**"Database offline" pill at top right (admin portal).**
...
**Login fails on customer site with "Invalid email or password".**
- Make sure you imported the latest `Airline_Database.sql`. Older versions didn't have the `password` column on Customer.
- All demo accounts use the password `pass123` (see the list above).
